...
export default function OrderSummary() {
  ...
  const handlePaymentClick = () => {
    ...
    mutateAsync({
      ...
      deposit_price: Math.floor(paymentObj.total)*100,
      deposit_charge: userDetails.deposit_charge,
      description: paymentDescription,
    })
      .then((res) => {
        toast.success("Redirecting to payment page")
        const link = `https://actions.stage.trustap.com/f2f/transactions/${res.id}/pay_deposit?redirect_uri=https://www.quickbooks-solutions.com/raise-transaction-dispute?transactionId=${res.id}&buyerId=${res.buyer_id}&state=${linkDetails}`
  ...
  const { price, edition, year, total, user, disc, discPer } = paymentObj
  const hasDiscount = disc && disc > 0
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <p className="font-semibold text-gray-900">${price*year}</p>
        ...
        {hasDiscount && (
          <div className="border border-dashed border-green-400 rounded-md bg-green-50 p-4 mt-4">
            ...
            <div className="mt-1 text-sm text-gray-800">
              <p>
                💸 <strong>{discPer}% off</strong> — You saved <strong>${disc}</strong> today.
              </p>
            </div>
          </div>
        )}
